...
public class SimpleObjectGlue extends CukeGlueAbstract {
    ...
    public void there_are_N_simple_objects(int n) throws Throwable {
        if(supportsMocks()) {
            checking(new Expectations() {
                {
                    allowing(service(Pets.class)).listAll();
                    will(returnValue(allSimpleObjects()));
                }
            });
        }
        try {
            final List<Pet> findAll = service(Pets.class).listAll();
    ...
    public void I_create_a_new_simple_object() throws Throwable {
        if(supportsMocks()) {
            checking(new Expectations() {
                {
                    oneOf(service(Pets.class)).create(with(any(String.class)));
                    will(addToInMemoryDB());
                }
            });
        }
        service(Pets.class).create(UUID.randomUUID().toString());
    }
    ...
    private Action addToInMemoryDB() {
        return new Action() {
            ...
            public Object invoke(Invocation invocation) throws Throwable {
                ...
                final Pet obj = new Pet();
                obj.setName(name);
                inMemoryDB.put(Pet.class, name, obj);
                return obj;
            }
    ...
    // helper
    private List<Pet> allSimpleObjects() {
        final InMemoryDB inMemoryDB = getVar("isis", "in-memory-db", InMemoryDB.class);
        return inMemoryDB.findAll(Pet.class);
    }
}
